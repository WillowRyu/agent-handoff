...
3. **Assess scope** *(skip if Gate matched Phase advance — phases already declared)*. If the work has clear, sequentially-dependent stages that are each independently verifiable (e.g., DB migration → API → frontend → cleanup), propose splitting into `## Phases`. Confirm with user before committing to a multi-phase plan. Otherwise proceed as a single-cycle plan.
